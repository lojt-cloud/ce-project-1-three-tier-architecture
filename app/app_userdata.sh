...
function checkTcpSocket(host, port, timeout = 2000) {
...
const server = http.createServer(async (req, res) => {
  ...
  // 2. Fetch Metadata & Probe DB Statuses
  ...
  const dbBadge = overallDbOk
    ? '<span style="background:#22c55e; color:#fff; padding:4px 12px; border-radius:12px; font-weight:bold;">CONNECTED (LIVE TCP OK)</span>'
...
<body>
  <div class="container">
    ...
    <div class="tier">
      ...
      <p style="font-size:0.8em;color:#64748b;">Standby offline by design -- single-AZ data tier due to free-tier vCPU quota. See IMPROVEMENTS.md.</p>
    </div>
  </div>
...
Environment=DB_HOST=10.0.21.10
Environment=STANDBY_DB_HOST=10.0.22.10
